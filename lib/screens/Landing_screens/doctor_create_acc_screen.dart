              ...
              Center(
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: const Color(0xFF2A6AD1).withAlpha(25),
...
class _PersonalInfoScreenState extends State<PersonalInfoScreen> {
  ...
  void initState() {
    super.initState();
    _registrationData = RegistrationData.forDoctor(
      name: '',
      email: '',
      phone: '',
      gender: 'Male',
      dob: '',
      password: '', // Provide a default or appropriate value for the password
      licenseNumber: '',
      specialization: '',
      workSetting: '',
      street: '',
  ...
  void _saveAndContinue() {
    if (_formKey.currentState!.validate()) {
      ...
      // Update RegistrationData object
      _registrationData = RegistrationData.forDoctor(
        name: _nameController.text,
        email: _emailController.text,
        phone: _phoneController.text,
        gender: 'Male', // Example: Update this based on user input
        dob: _dobController.text,
        licenseNumber: _registrationData.licenseNumber ?? '',
        specialization: _registrationData.specialization ?? '',
        workSetting: _registrationData.workSetting ?? '',
        street: _registrationData.street ?? '',
        city: _registrationData.city ?? '',
        state: _registrationData.state ?? '',
        zip: _registrationData.zip ?? '',
        hasClinic: _registrationData.hasClinic ?? false,
        clinicName: _registrationData.clinicName,
        clinicAddress: _registrationData.clinicAddress, 
        password: '',
      );
...
class _ProfessionalInfoScreenState extends State<ProfessionalInfoScreen> {
  ...
  if (_formKey.currentState!.validate()) {
    ...
    // Create a new RegistrationData object with updated fields
    final updatedRegistrationData = RegistrationData.forDoctor(
      ...
      workSetting: _selectedWorkSetting,
      street: widget.registrationData.street ?? '',
      city: widget.registrationData.city ?? '',
      state: widget.registrationData.state ?? '',
      zip: widget.registrationData.zip ?? '',
      hasClinic: widget.registrationData.hasClinic ?? false,
      clinicName: widget.registrationData.clinicName,
      clinicAddress: widget.registrationData.clinicAddress,
    );

    // Navigate to the next screen with the updated RegistrationData object
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => AddressScreen(registrationData: updatedRegistrationData),
      ),
    );
  }
}
...
class _AddressScreenState extends State<AddressScreen> {
  ...
  void _saveAndContinue() {
    if (_formKey.currentState!.validate()) {
      ...
      // Create an updated RegistrationData object using the factory method
      final updatedRegistrationData = RegistrationData.forDoctor(
        name: widget.registrationData.name,
        email: widget.registrationData.email,
        phone: widget.registrationData.phone,
        ...
        specialization: widget.registrationData.specialization ?? '',
        workSetting: widget.registrationData.workSetting ?? '',
        street: _streetController.text,
        city: _cityController.text,
        state: _stateController.text,
        zip: _zipController.text,
        hasClinic: _hasClinic,
        clinicName: _hasClinic ? _clinicNameController.text : null,
        clinicAddress: _hasClinic ? _clinicAddressController.text : null,
      );
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => PasswordScreen(registrationData: updatedRegistrationData),
        ),
      );
    }
  }
  ...
  Widget _buildSectionHeader(String title) {
    return Column(
      ...
      children: [
        Text(
          title,
          style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Color(0xFF2A6AD1)),
        ),
        const SizedBox(height: 4),
        Container(
          width: 50,
...
class PasswordScreen extends StatefulWidget {
  final RegistrationData registrationData;
  const PasswordScreen({super.key, required this.registrationData});
...
class _PasswordScreenState extends State<PasswordScreen> {
  ...
  if (_formKey.currentState!.validate() && _agreedToTerms) {
    ...
    // Create an updated RegistrationData object with the new password
    final updatedRegistrationData = RegistrationData.forDoctor(
      name: widget.registrationData.name,
      email: widget.registrationData.email,
      phone: widget.registrationData.phone,
      gender: widget.registrationData.gender,
      dob: widget.registrationData.dob,
      password: _passwordController.text, // Update the password
      licenseNumber: widget.registrationData.licenseNumber ?? '',
      specialization: widget.registrationData.specialization ?? '',
      workSetting: widget.registrationData.workSetting ?? '',
      street: widget.registrationData.street ?? '',
    ...
    try {
      // Call Supabase service to register the doctor
      await _supabaseService.registerDoctor(updatedRegistrationData);
      ...
      if (mounted) {
        Navigator.pushAndRemoveUntil(
          context,
          MaterialPageRoute(
            builder: (context) => RegistrationSuccessScreen(uniqueId: 'your_unique_id_here'),
          ),
          (route) => false,
        );
      }
    ...
    // Show error message if terms are not agreed to
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(

...
class _PatientDashboardState extends State<PatientDashboard>
  ...
  Future<void> _fetchPatientData() async {
    try {
      print('Fetching data for patient ID: ${widget.patientId}');
      final patientResponse = await Supabase.instance.client
          .from('patients')
          .select('*')
          .eq('unique_id', widget.patientId)
          .single();
      print('Patient Response: $patientResponse');

      List<Map<String, dynamic>> medicalRecordsResponse = [];
      List<Map<String, dynamic>> medicationsResponse = [];
      List<Map<String, dynamic>> labTestsResponse = [];

      try {
        // Fetch medical records (optional)
        medicalRecordsResponse = await Supabase.instance.client
      ...
      try {
        // Fetch current medications (optional)
        medicationsResponse = await Supabase.instance.client
            .from('medications')
            .select('*')
      ...
      try {
        // Fetch recent lab tests (optional)
        labTestsResponse = await Supabase.instance.client
            .from('lab_tests')
            .select('*')
            .eq('patient_id', widget.patientId)
      ...
      setState(() {
        patientInfo = {
          'name': patientResponse['name'] ?? 'Patient',
          'id': patientResponse['unique_id'] ?? 'N/A',
          'age': _calculateAge(patientResponse['dob']),
          'bloodGroup': patientResponse['blood_group'] ?? 'N/A',
          'email': patientResponse['email'] ?? 'N/A',
        };
        _isLoading = false;
        print('State updated. Patient Info: $patientInfo');

        medicalRecords = (medicalRecordsResponse as List).map((record) {
          return {
            'condition': record['condition'] ?? 'Unknown',
            'status': record['status'] ?? 'Ongoing',
            'icon': _getIconForCondition(record['condition']),
            'color': _getColorForCondition(record['condition']),
            'spine_color': _getSpineColorForCondition(record['condition']),
            'pages_color': Colors.white,
          };
        }).toList();

        medications = (medicationsResponse as List).map((med) {
          return {
            'name': med['medication_name'] ?? 'Medication',
            'dosage': '${med['dosage'] ?? ''}${med['dosage_unit'] ?? ''}',
            'time': med['scheduled_time'] ?? 'N/A',
            'status': med['status'] ?? 'Pending',
            'icon': med['status'] == 'Taken'
                ? Icons.check_circle
                : Icons.access_time,
            'statusColor': med['status'] == 'Taken'
        ...
        // Transform lab tests
        labTests = (labTestsResponse as List).map((test) {
          return {
            'name': test['test_name'] ?? 'Lab Test',
            'date': _formatDate(test['test_date']),
            'status': test['status'] ?? 'Completed',
            'result': test['result'] ?? 'Pending',
            'resultColor': _getResultColor(test['result']),
          };
        }).toList();
  ...
  // Method to dynamically assign icons based on condition
  ...
  // Method to dynamically assign colors based on condition
  ...
  Color _getSpineColorForCondition(String? condition) {
    switch (condition?.toLowerCase()) {
      case 'heart condition':
                  ...
                  Positioned(
                    ...
                    child: AnimatedBuilder(
                      ...
                      builder: (context, child) {
                        return Transform.scale(
                          scale:
                              1.0 + _dietPlanAnimationController.value * 0.08,
                          child: Opacity(
                            opacity: 0.12,
                            child: Container(
                              width: 100,
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            buildLabTests(),
                            const SizedBox(
                                height: 80), // Extra space for chatbot
                          ],
                        ),
                      ),
                    ),
              ...
              children: [
                ...
                Container(
                  ...
                  child: const CircleAvatar(
                    ...
                    child:
                        Icon(Icons.person, color: Color(0xFF4285F4), size: 32),
                  ),
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        'Hello ${patientInfo['name'] != null ? patientInfo['name'].split(' ')[0] : 'Patient'}!',
  ...
  Widget _buildPatientInfoBadge(String text, IconData icon,
          ...
          return AnimatedBuilder(
            ...
            builder: (context, child) {
              ...
              final animationValue =
                  (_bookAnimationController.value - delay).clamp(0.0, 1.0) /
                      (1.0 - delay);
              ...
              return Positioned(
                ...
                child: Transform(
                  transform: Matrix4.identity()
                    ...
                    ..rotateZ((rotationZ -
                            _bookRotationAnimation.value * recordIndex) *
                        (1 - animationValue * 0.7))
  ...
  Widget _buildEnhancedBookCard(String condition, String status, IconData icon,
      Color color, Color spineColor, Color pagesColor, int index) {
        ...
        return GestureDetector(
          ...
          child: AnimatedBuilder(
            ...
            builder: (context, child) {
              ...
              final delayedValue =
                  (_pulseAnimationController.value - (index * 0.2))
                      .clamp(0.0, 1.0);
              final scale = 1.0 +
                  (delayedValue > 0.5 ? _pulseAnimation.value - 1.0 : 0.0) *
                      0.5;
              return Transform.scale(
                scale: scale,
                child: child,
